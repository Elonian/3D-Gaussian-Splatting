...
class GSRasterizer(object):
    ...
    def __init__(self):
        ...
        self.tile_size = 25
        self.frustum_guard = None
        self.max_splat_radius = None
    ...
    def render_scene(self, scene: Scene, camera: Camera):
        ...
        if self.frustum_guard is not None:
            guard = float(self.frustum_guard)
            in_mask = in_mask & (mean_ndc[:, 0].abs() <= guard) & (mean_ndc[:, 1].abs() <= guard)
        ...
        det_2d = cov_2d[:, 0, 0] * cov_2d[:, 1, 1] - cov_2d[:, 0, 1] * cov_2d[:, 1, 0]
        valid_mask = (
            torch.isfinite(cov_2d).flatten(1).all(dim=1)
            & torch.isfinite(mean_ndc).all(dim=1)
            & torch.isfinite(mean_view).all(dim=1)
        ...
        assert valid_mask.any(), "No valid projected Gaussians"
        mean_ndc = mean_ndc[valid_mask]
        mean_view = mean_view[valid_mask]
    ...
    def compute_cov_2d(
        ...
        mean_view = homogenize(mean_3d) @ w2c
        t_x = mean_view[:, 0]
        t_y = mean_view[:, 1]
        t_z = mean_view[:, 2]
        # ========================================================
    ...
    def render(
        ...
        valid_radius = torch.isfinite(radii)
        if self.max_splat_radius is not None:
            valid_radius = valid_radius & (radii <= float(self.max_splat_radius))
        assert valid_radius.any(), "No valid projected splat radii"
        mean_2d = mean_2d[valid_radius]
        cov_2d = cov_2d[valid_radius]
        color = color[valid_radius]
        opacities = opacities[valid_radius]
        depths = depths[valid_radius]
        radii = radii[valid_radius]
        rect = get_rect(mean_2d, radii, width=camera.image_width, height=camera.image_height)

        ys, xs = torch.meshgrid(
            torch.arange(camera.image_height, device=mean_2d.device, dtype=mean_2d.dtype),
            torch.arange(camera.image_width, device=mean_2d.device, dtype=mean_2d.dtype),
            indexing='ij',
        )
        ...
        if self.white_bkgd:
            render_color = torch.ones(*pix_coord.shape[:2], 3, device=mean_2d.device, dtype=mean_2d.dtype)
        else:
            render_color = torch.zeros(*pix_coord.shape[:2], 3, device=mean_2d.device, dtype=mean_2d.dtype)
        ...
        for h in range(0, camera.image_height, self.tile_size):
            for w in range(0, camera.image_width, self.tile_size):
                ...
                sort_idx = torch.argsort(depths[in_mask])
                mean_2d_tile = mean_2d[in_mask][sort_idx]
                cov_2d_tile = cov_2d[in_mask][sort_idx]
                color_tile = color[in_mask][sort_idx]
                opacity_tile = opacities[in_mask][sort_idx]
                # ========================================================
                
                # ========================================================
                # TODO: Compute the displacement vector from the 2D mean coordinates to the pixel coordinates
                pix_tile = pix_coord[h:h+self.tile_size, w:w+self.tile_size].reshape(-1, 2)
                disp = pix_tile[:, None, :] - mean_2d_tile[None, :, :]
                # ========================================================

                # ========================================================
                # TODO: Compute the Gaussian weight for each pixel in the tile
                cov_inv = torch.linalg.inv(cov_2d_tile)
                power = torch.einsum("pgi,gij,pgj->pg", disp, cov_inv, disp)
                power = torch.nan_to_num(power, nan=float("inf"), posinf=float("inf"), neginf=float("inf"))
                weight = torch.exp(-0.5 * torch.clamp(power, min=0.0))
                # ========================================================

                # ========================================================
                # TODO: Perform alpha blending
                alpha = torch.clamp(weight * opacity_tile.reshape(1, -1), min=0.0, max=0.99)
                trans = torch.cumprod(
                    torch.cat([torch.ones_like(alpha[:, :1]), 1.0 - alpha], dim=1),
                    dim=1,
                )[:, :-1]
                alpha_weight = alpha * trans
                tile_color = (alpha_weight[..., None] * color_tile[None]).sum(dim=1)
                if self.white_bkgd:
                    final_trans = torch.prod(1.0 - alpha, dim=1, keepdim=True)
                    tile_color = tile_color + final_trans
                # ========================================================
