...
class Args:
    ...
    out_root: Path = PROJECT_ROOT / "outputs"
    """Root directory for saving outputs."""
    num_views: int | None = None
    """Optional number of camera views to render. Defaults to all views."""
...
def main(args: Args):
    ...
    num_views = len(c2ws) if args.num_views is None else min(args.num_views, len(c2ws))
    try:
        for view_idx, c2w in tqdm(enumerate(c2ws[:num_views]), total=num_views):
...
def load_camera_params(scene_type, device, use_half):
    ...
    """
    data_root = PROJECT_ROOT / "data" / "nerf_synthetic" / scene_type
    ...
    for frame in tr_dict["frames"]:
        ...
        c2ws.append(c2w)
    c2ws = np.array(c2ws)
    first_img = imageio.imread(data_root / (tr_dict["frames"][0]["file_path"] + ".png"))
...
def load_scene(scene_type, device):
    ply_path = PROJECT_ROOT / "data" / f"{scene_type}.ply"
